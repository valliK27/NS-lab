...
main (int argc, char *argv[])
{
  ...
  // Create the OnOff application to send UDP datagrams of size //just add sinks
  // 512 bytes (default) at a rate of 500 Kb/s (default) from n0
  NS_LOG_INFO ("Create Applications.");
  OnOffHelper onoff ("ns3::UdpSocketFactory",  //sending the packets
  ...
  //in broadcast you dont need to set the route

  // Create an optional packet sink to receive these packets
  PacketSinkHelper sink ("ns3::UdpSocketFactory",
                         Address (InetSocketAddress (Ipv4Address::GetAny (), port)));
  app = sink.Install (c0.Get (1)); //first node is the sink n1
  app.Add (sink.Install (c1.Get (1)));//second node is also the sink n2 //adding two sinks
  app.Start (Seconds (1.0));
  app.Stop (Seconds (10.0));
  ...
  AnimationInterface anim("MTl4boradcast.xml");
