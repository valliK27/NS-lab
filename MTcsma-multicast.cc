...
main (int argc, char *argv[])
{
  ...
  NodeContainer c0 = NodeContainer (c.Get (0), c.Get (1), c.Get (2)); //lan 0
  NodeContainer c1 = NodeContainer (c.Get (2), c.Get (3), c.Get (4)); //lan 1 //see that 2 is repeated
  ...
  ipv4Addr.SetBase ("10.1.1.0", "255.255.255.0"); // LAN 0 address
  ipv4Addr.Assign (nd0);
  ipv4Addr.SetBase ("10.1.2.0", "255.255.255.0"); // LAN1 address
  ipv4Addr.Assign (nd1);
  ...
  Ptr<Node> multicastRouter = c.Get (2);  // The node in question //the joining point
  ...
  OnOffHelper onoff ("ns3::UdpSocketFactory", 
                     Address (InetSocketAddress (multicastGroup, multicastPort))); //sender keeps sending the packets  to the mulicast group
  ...
  ApplicationContainer sinkC = sink.Install (c1.Get (2)); // Node n4  is receiving all the packets
  // Start the sink
  ...
  AnimationInterface anim("MTl4multicast.xml");
